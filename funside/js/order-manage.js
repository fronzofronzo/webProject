...
async function init() {
        ...
        for(let i=0; i<orders.length; i++) {
            ...
            <div class="d-flex p-3 align-items-center order-manage mt-4">  
                ...
                <div class="d-flex justify-content-end flex-grow-1" >
                    <button class="btn btn-primary me-2 statusButton" id="${orders[i]["idorder"]}"  value ="${nextStatus(orders[i]["status"])}" ${orders[i]["suspended"]  }>Passa a stato successivo</button>
                    <button class="btn btn-danger suspendButton" id=${orders[i]["idorder"]}">Sospendi</button>
                </div>
             </div>
            `;
        }
    ...
    statusButtons.forEach(btn => 
        btn.addEventListener("click", function(e) {
            e.preventDefault();
            console.log("Pressed order: " + btn.getAttribute("id"));
    }));
}
